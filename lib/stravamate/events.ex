defmodule Stravamate.Events do
  @moduledoc """
  Events context
  """
  ...
  alias Stravamate.Events.{Event, Team, EventTeamUser}
  ...
  def refresh_cache(%Event{id: id} = _event, sync \\ false) do
    case sync do
      false ->
        %{event_id: id}
        |> Stravamate.EventRefreshCache.new()
        |> Oban.insert()

      true ->
        Stravamate.EventRefreshCache.perform(%Oban.Job{args: %{"event_id" => id}})
    end
  end
end
